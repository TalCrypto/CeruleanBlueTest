...
import { PrimaryNFT, SecondaryNFT } from "../typechain-types";
...
describe("test", function () {
  ...
  it("deployment", async function () {
    ...
    pToken = (await upgrades.deployProxy(primaryNFT, ["PrimaryNFT", "PNFT", "https://example.com/"])) as PrimaryNFT;
    await pToken.deployed();

    const secondaryNFT = await ethers.getContractFactory("SecondaryNFT");
    sToken = (await upgrades.deployProxy(secondaryNFT, ["SecondaryNFT", "SNFT", "https://example123.com/", pToken.address])) as SecondaryNFT;
  ...
  describe("Transactions", function () {
    ...
    it("should success when minting a primary nft", async function () {
      await pToken.connect(player).mint({value: ethers.utils.parseEther("0.01")});
    ...
    it("should own a secondary nft after escrowing the primary nft", async function () {
      await pToken.connect(player)["safeTransferFrom(address,address,uint256)"](player.address, sToken.address, 0);
    ...
    it("should fail when swapping with other's secondary nft", async function () {
      await expect(
        sToken.connect(hacker).swap(0)
      ).to.be.revertedWith("Not an owner");
    });

    it("should fail when swapping with other wallet", async function () {
      await sToken.connect(player).transferFrom(player.address, hacker.address, 0);
      await expect(
        sToken.connect(hacker).swap(0)
      ).to.be.revertedWith("Not escrowed for this nft");
      await sToken.connect(hacker).transferFrom(hacker.address, player.address, 0);
    });
